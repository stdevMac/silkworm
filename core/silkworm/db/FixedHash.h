...
#ifndef SILKWORM_FIXEDHASH_H
#define SILKWORM_FIXEDHASH_H
...
class FixedHash {
  public:
    ...
    explicit FixedHash(std::string const& _s, ConstructFromStringType _t = FromHex,
        ...
        : FixedHash(_t == FromHex ? fromHex(_s) : silkworm::db::asBytes(_s), _ht) {}
    ...
    [[nodiscard]] byte const* data() const { return m_data.data(); }
    ...
    struct hash {
        ...
        size_t operator()(FixedHash const& _value) const {
            (void)_value.ref();
            return 0;  // boost::hash_range(_value.m_data.cbegin(), _value.m_data.cend());
        }
    };
    ...
    inline FixedHash<M> bloomPart() const {
        ...
        for (unsigned i = 0; i < P; ++i) {
            unsigned index = 0;
            for (unsigned j = 0; j < c_bloomBytes; ++j, ++p) index = (index << 8) | *p;
            index &= c_mask;
...
class SecureFixedHash : private FixedHash<T> {
  public:
    ...
    explicit SecureFixedHash(bytesConstRef _b, ConstructFromHashType _t = FixedHash<T>::FailIfDifferent)
        : FixedHash<T>(_b, _t) {}
    explicit SecureFixedHash(bytesSec const& _b, ConstructFromHashType _t = FixedHash<T>::FailIfDifferent)
    ...
    explicit SecureFixedHash(std::string const& _s, ConstructFromStringType _t = FixedHash<T>::FromHex,
...
//     const uint64_t* hash1 = std::reinterpret_cast<uint64_t*>(data());
//     uint64_t* hash2 = (const uint64_t*)_other.data();
...
inline size_t FixedHash<32>::hash::operator()(FixedHash<32> const& value) const {
    ...
    return size_t(data);  // boost::hash_range(data, data + 4);
}
...
inline std::string toString(h256s const& _bs) {
    std::string out = "[ ";
    for (h256 const& i : _bs) out.append(i.abridged() + ", ");
    out.append("]");
    return out;
}
...
template <>
struct hash<silkworm::db::h64> : silkworm::db::h64::hash {};
template <>
struct hash<silkworm::db::h128> : silkworm::db::h128::hash {};
template <>
struct hash<silkworm::db::h160> : silkworm::db::h160::hash {};
template <>
struct hash<silkworm::db::h256> : silkworm::db::h256::hash {};
template <>
struct hash<silkworm::db::h512> : silkworm::db::h512::hash {};
}  // namespace std

#endif  // SILKWORM_FIXEDHASH_H
