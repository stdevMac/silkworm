...
std::string hexPrefixEncode(bytes const& _hexVector, bool _leaf, int _begin, int _end) {
    auto begin = unsigned(_begin);
    unsigned end = _end < 0 ? _hexVector.size() + 1 + unsigned(_end) : unsigned(_end);
    ...
    for (unsigned i = begin; i < end; i += 2) ret += char(_hexVector[i] * 16 + _hexVector[i + 1]);
    return ret;
}

std::string hexPrefixEncode(bytesConstRef _data, bool _leaf, int _beginNibble, int _endNibble, unsigned _offset) {
    unsigned begin = unsigned(_beginNibble) + _offset;
    unsigned end =
        (_endNibble < 0 ? (unsigned(_data.size() * 2 - _offset) + 1) + unsigned(_endNibble) : unsigned(_endNibble)) +
        _offset;
