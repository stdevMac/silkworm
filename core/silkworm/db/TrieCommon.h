...
#ifndef SILKWORM_TRIECOMMON_H
#define SILKWORM_TRIECOMMON_H
...
inline unsigned sharedNibbles(bytesConstRef _first, unsigned _beginFirst, unsigned _endFirst, bytesConstRef _second,
                              unsigned _beginSecond, unsigned _endSecond) {
    unsigned ret = 0;
...
struct NibbleSlice {
    ...
    [[nodiscard]] unsigned size() const { return data.size() * 2 - offset; }
    [[nodiscard]] bool empty() const { return !size(); }
    [[nodiscard]] NibbleSlice mid(unsigned _index) const { return NibbleSlice(data, offset + _index); }
    void clear() {
        data.reset();
        offset = 0;
    }
    ...
    [[nodiscard]] bool contains(NibbleSlice _k) const { return shared(_k) == _k.size(); }
    /// @returns the number of shared nibbles at the beginning of this and _k.
    [[nodiscard]] unsigned shared(NibbleSlice _k) const {
     ...
     */
    [[nodiscard]] bool isEarlierThan(NibbleSlice _k) const {
        unsigned i = 0;
...
inline std::ostream& operator<<(std::ostream& _out, NibbleSlice const& _m) {
    for (unsigned i = 0; i < _m.size(); ++i) _out << std::hex << int(_m[i]) << std::dec;
    return _out;
}
...
inline NibbleSlice keyOf(bytesConstRef _hpe) {
    if (_hpe.empty()) return NibbleSlice(_hpe, 0);
    if (_hpe[0] & 0x10)
...
inline NibbleSlice keyOf(RLP const& _twoItem) { return keyOf(_twoItem[0].payload()); }
